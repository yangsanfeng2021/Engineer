...
public class TabView extends FrameLayout {
    ...
    private void initView(Context context) {
        itemView = LayoutInflater.from(context).inflate(R.layout.tab_view, this, false);
        addView(itemView);
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        itemTitleColor = typedArray.getColor(R.styleable.TabView_itemTitleColor, Color.parseColor("#8d8d8d"));
        itemTitleColorSelected = typedArray.getColor(R.styleable.TabView_itemTitleColorSelected, Color.parseColor("#11d7aa"));
        itemStateSelected = typedArray.getBoolean(R.styleable.TabView_itemStateSelected, false);
        setItemStateSelected(itemStateSelected);
        dotVisible = typedArray.getBoolean(R.styleable.TabView_dotVisible, false);
        setDotVisible(dotVisible);
        unreadNumVisible = typedArray.getBoolean(R.styleable.TabView_unreadNumVisible, false);
        setUnreadNumVisible(unreadNumVisible);
        unreadNum = typedArray.getString(R.styleable.TabView_unreadNum);
        ...
        itemView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (onTabItemStateChangeListener != null) {
                    boolean itemState = onTabItemStateChangeListener.shouldChangeTabItemState(TabView.this);
                    if (itemState) {
                        setItemStateSelected(itemStateSelected);
                        onTabItemStateChangeListener.onTabItemStateChanged(TabView.this);
                    }
                } else {
                    setItemStateSelected(!itemStateSelected);
                }
    ...
    public interface OnTabItemStateChangeListener {
        // 是否应该改变
        boolean shouldChangeTabItemState(TabView tabView);

        // 已经改变
        void onTabItemStateChanged(TabView tabView);
    }
}
